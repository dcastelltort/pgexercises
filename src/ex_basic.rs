...
pub fn basic_selectall() -> (Vec<Facility>, Vec<Facility>) {
    ...
    //DSL
    let results: Vec<Facility> = facilities
        .load(&connection)
...
pub fn basic_select_specific() -> (Vec<FacilityPartial>, Vec<FacilityPartial>) {
    ...
    //DSL
    let results: Vec<FacilityPartial> = facilities
...
pub fn basic_select_where2() -> (Vec<FacilityPartial4>, Vec<FacilityPartial4>) {
    ...
    //DSL
    let results: Vec<FacilityPartial4> = facilities
...
pub fn basic_classify() -> (Vec<StringTuple>, Vec<StringTuple>) {
    ...
    let results_sql: Vec<(String, String)> = intermediate_sql
        .into_iter()
        .map(|res: FacilityPartial2| {
            let cost = if res.monthlymaintenance < BigDecimal::from(100) {
                String::from("cheap")
            } else {
    ...
    //DSL
    let intermediate_results: Vec<(String, BigDecimal)> = facilities
        .select((name, monthlymaintenance))
        .load::<(String, BigDecimal)>(&connection)
        .expect("diesel operation failed");

    let results: Vec<(String, String)> = intermediate_results
        .into_iter()
        .map(|res: (String, BigDecimal)| {
            let cost = if res.1 < BigDecimal::from(100) {
                String::from("cheap")
            } else {
                String::from("expensive")
            };
...
pub fn basic_date() -> (Vec<Member4>, Vec<Member4>) {
    ...
    let results_sql : Vec<Member4> = sql_query("SELECT memid, surname, firstname, joindate FROM members WHERE joindate > TIMESTAMP '2012-09-01'")
        ...
        .expect("query failed to run");
    //DSL
    let results: Vec<Member4> = members
...
pub fn basic_union() -> (Vec<String>, Vec<String>) {
    ...
    let results_members: Vec<String> = members
        .select(surname)
        .load::<String>(&connection)
        .expect("diesel operation failed");

    let results_facilities: Vec<String> = facilities
        .select(name)
        .load::<String>(&connection)
        .expect("diesel operation failed");
...
pub fn basic_agg() -> (Vec<String>, Vec<String>) {
    ...
    let results_sql: Vec<String> = sql_query("SELECT MAX(joindate) as joindate FROM members")
        ...
        .expect("query failed to run")
        .iter()
        .map(|member| member.joindate.format("%Y-%m-%d %H:%M:%S").to_string())
        .collect();
    //DSL
    let results_date: Option<NaiveDateTime> = members
        .select(max(joindate))
...
pub fn basic_agg2() -> (Vec<Member3>, Vec<Member3>) {
    ...
    let results_sql : Vec<Member3> = sql_query("SELECT firstname, surname, joindate FROM members WHERE joindate = (SELECT MAX(joindate) FROM members);")
        ...
        .expect("query failed to run");
    //DSL
    let results: Vec<Member3> = members
